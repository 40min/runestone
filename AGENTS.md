...
- The `safety-dependency-scan` pre-commit hook is manual-only. Set `SAFETY_API_KEY` and run `make security-check` for a local security sweep; CI also runs `pre-commit run safety --hook-stage manual --all-files` with `SAFETY_REQUIRED=1` on opened, synchronized, or reopened pull requests and pushes to `main`, failing closed when the key is missing. Keep the key in the environment rather than command arguments or logs.
...
- Treat `docs/error-telemetry.md` as the source of truth for Better Stack/Sentry changes: keep the default-deny allowlist and use `capture_sanitized_exception()` for handled boundary failures. Before the broader readiness gate, run the focused coverage with `UV_CACHE_DIR=.uv-cache uv run --extra dev pytest tests/core/test_error_tracking.py tests/api/test_auth_telemetry.py tests/api/test_recall_endpoints.py tests/test_recall_main.py -v`.
...
- For PWA changes, ordinary Vite development keeps service-worker generation disabled; use `cd frontend && BUILD_PWA=1 npm run build` for a production-style local check, inspect `frontend/dist/manifest.webmanifest` and the generated worker, and serve that build/container for install and offline checks. PWA v1 caches only the static shell: `/api/`, uploads, WebSockets, and media remain network-only; deployed HTTPS installation is a separate smoke test from repository readiness.
...
- If recall delivery schedules need extra browser or container/timezone evidence, use the guarded helpers in `integration_tests/browser/` or `integration_tests/containers/`; these optional checks are separate from `make check-readiness`.
