...
- For manual broader sweeps outside the standard gate, prefer the composed Make workflows: `make dev-test`, `make dev-full`, `make ci-lint`, and `make ci-test`. When you specifically want the standard aggregate entry points, use `make lint` for auto-fix linting across backend and frontend, `make test` for both test suites without coverage, and `make test-coverage` when you need the backend coverage report plus the frontend test run.
