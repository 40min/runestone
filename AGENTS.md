...
- Add final-stage checks only when they match the change: run `make frontend-lockfile-sync` when you need to regenerate `frontend/package-lock.json`, then `make frontend-lockfile-check` after changing `frontend/package.json` or `frontend/package-lock.json`, and run `make security-check` before sending changes that touch dependencies, security-sensitive code paths, or auth/configuration surfaces.
