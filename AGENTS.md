...
- While iterating on a change, prefer the narrowest checks available so feedback stays fast. In practice, that usually means targeted `uv run pytest tests/path -v` for backend work rather than repo-wide or area-wide Make targets.
- Before opening a PR, pushing a review-round update, or otherwise sending changes upstream, run the broader validation pass. Start with `make check-readiness` for the standard repo-wide gate: lint checks, backend and frontend tests, and a frontend build dry-run. Use `make backend-lint`, `make frontend-lint`, `make backend-test`, or `make frontend-test` only when you specifically need those broader per-surface checks outside the full gate.
- Add final-stage checks only when they match the change: run `make frontend-lockfile-check` after changing `frontend/package.json` or `frontend/package-lock.json`, and run `make security-check` before sending changes that touch dependencies, security-sensitive code paths, or auth/configuration surfaces.
