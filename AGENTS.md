...
- For authenticated browser evidence of recall delivery schedules, follow E6 in `docs/todo/user-profile-recall-timezone.md`: prepare disposable accounts with `UV_CACHE_DIR=.uv-cache uv run --extra dev python integration_tests/browser/prepare_recall_schedule_fixture.py --database-url "$BROWSER_TEST_DATABASE_URL" --output-dir "$BROWSER_EVIDENCE_DIR"`, run the two headed `playwright-cli` sessions against `DATABASE_URL="$BROWSER_TEST_DATABASE_URL" make run-dev`, and always clean up with `--cleanup-manifest "$BROWSER_EVIDENCE_DIR/fixture-manifest.json"`. Credentials stay in the helper's mode-0600 evidence file and this gate is separate from `make check-readiness`.
