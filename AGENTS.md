...
- The `safety-dependency-scan` pre-commit hook is manual-only. Set `SAFETY_API_KEY` and run `make security-check` for a local security sweep; CI also runs `pre-commit run safety --hook-stage manual --all-files` with `SAFETY_REQUIRED=1` and fails closed when the key is missing. Keep the key in the environment rather than command arguments or logs.
...
- For authenticated browser evidence of recall delivery schedules, prepare disposable accounts with `UV_CACHE_DIR=.uv-cache uv run --extra dev python integration_tests/browser/prepare_recall_schedule_fixture.py --database-url "$BROWSER_TEST_DATABASE_URL" --output-dir "$BROWSER_EVIDENCE_DIR"`, run the two headed `playwright-cli` verifiers against `DATABASE_URL="$BROWSER_TEST_DATABASE_URL" make run-dev`, and always clean up with `--cleanup-manifest "$BROWSER_EVIDENCE_DIR/fixture-manifest.json"`. The helper accepts only loopback PostgreSQL databases ending in `_test`, stores evidence credentials privately, and this gate is separate from `make check-readiness`.
- For isolated container and timezone startup evidence, run `UV_CACHE_DIR=.uv-cache uv run --extra dev python integration_tests/containers/verify_recall_schedule_startup.py`; it uses the test-only Compose file, rejects non-local PostgreSQL configuration, records redacted evidence under `/tmp`, and removes its temporary volume in `finally`. This is separate from the normal readiness gate.
