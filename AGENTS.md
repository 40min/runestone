...
- For CLI and prompt-debugging workflows, use `make run IMAGE_PATH=...`, `make load-vocab CSV_PATH=...` with optional `DB_NAME=...` and `SKIP_EXISTENCE_CHECK=true`, and the prompt targets `make test-prompts-ocr`, `make test-prompts-analysis TEXT=...`, `make test-prompts-vocabulary WORD=...` with optional `MODE=example_only|extra_info_only|all_fields`, and `make test-grammar-search QUERY=...`.
