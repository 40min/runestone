...
class TestRunestoneIntegration:
    ...
    @patch('runestone.core.ocr.OCRProcessor')
    @patch('runestone.core.analyzer.ContentAnalyzer')
    @patch('runestone.core.formatter.ResultFormatter')
    def test_processor_init_success(self, mock_formatter, mock_analyzer, mock_ocr):
        """Test successful processor initialization."""
        processor = RunestoneProcessor(self.api_key, verbose=True)
        
        assert processor.verbose is True
        mock_ocr.assert_called_once_with(self.api_key, True)
        mock_analyzer.assert_called_once_with(self.api_key, True)
    ...
    def test_processor_init_failure(self, mock_ocr):
        ...
        with pytest.raises(RunestoneError) as exc_info:
            RunestoneProcessor(self.api_key)
    ...
    @patch('runestone.core.analyzer.ContentAnalyzer')
    @patch('runestone.core.formatter.ResultFormatter')
    def test_process_image_complete_workflow(self, mock_formatter, mock_analyzer, mock_ocr):
    ...
    def test_process_image_ocr_failure(self, mock_formatter, mock_analyzer, mock_ocr):
        ...
        processor = RunestoneProcessor(self.api_key)
    ...
    @patch('runestone.core.analyzer.ContentAnalyzer')
    @patch('runestone.core.formatter.ResultFormatter')
    def test_process_image_empty_text(self, mock_formatter, mock_analyzer, mock_ocr):
