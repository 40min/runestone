...
@pytest.mark.anyio
async def test_update_memory_priority_unset_calls_service():
    user = SimpleNamespace(id=42)
    mock_service = MagicMock()
    mock_service.update_item_priority = AsyncMock(return_value=SimpleNamespace(id=7, key="verb_tense", priority=None))
