...
class TestDependencyProviders:
    ...
    def test_get_llm_model(self):
        """Test get_llm_model access from app state."""
        mock_request = Mock()
        expected_model = Mock(spec=BaseChatModel)
        mock_request.app.state.llm_model = expected_model

        result = get_llm_model(mock_request)
    ...
    def test_get_content_analyzer(self, mock_content_analyzer_class):
        ...
        mock_llm_model = Mock(spec=BaseChatModel)
        mock_analyzer = Mock(spec=ContentAnalyzer)
        mock_content_analyzer_class.return_value = mock_analyzer

        # Execute
        result = get_content_analyzer(mock_settings, mock_llm_model)
    ...
    def test_get_ocr_llm_model(self):
        """Test get_ocr_llm_model access from app state."""
        mock_request = Mock()
        expected_model = Mock(spec=BaseChatModel)
        mock_request.app.state.ocr_llm_model = expected_model

        result = get_ocr_llm_model(mock_request)
    ...
    def test_get_ocr_processor(self, mock_ocr_processor_class):
        ...
        # Setup
        mock_settings = Mock(spec=Settings)
        mock_ocr_llm_model = Mock(spec=BaseChatModel)
        mock_processor = Mock(spec=OCRProcessor)
        mock_ocr_processor_class.return_value = mock_processor

        # Execute
        result = get_ocr_processor(mock_settings, mock_ocr_llm_model)
    ...
    def test_get_vocabulary_service(self, mock_vocabulary_service_class):
        ...
        # Execute
        result = get_vocabulary_service(mock_repo, mock_settings, mock_llm_model)
