...
class TestGrammarEndpoints:
    ...
    async def test_list_cheatsheets_success(self, client_with_mock_grammar_service, temp_cheatsheets_dir):
        ...
        response = await client.get("/api/grammar/cheatsheets")
    ...
    async def test_list_cheatsheets_empty(self, client_with_mock_grammar_service):
    ...
    async def test_list_cheatsheets_service_error(self, client_with_mock_grammar_service):
    ...
    async def test_get_cheatsheet_content_success(self, client_with_mock_grammar_service):
        """Test successful retrieval of cheatsheet content."""
        client, mock_service = client_with_mock_grammar_service
        mock_service.get_cheatsheet_content.return_value = "# Test Content\n\nThis is test content."

        response = await client.get("/api/grammar/cheatsheets/adjectiv-komparation.md")
    ...
    async def test_get_cheatsheet_content_nested_path_success(self, client_with_mock_grammar_service):
        ...
        response = await client.get("/api/grammar/cheatsheets/verbs/hjalpverb.md")
    ...
    async def test_get_cheatsheet_content_invalid_filepath(self, client_with_mock_grammar_service):
        ...
        response = await client.get("/api/grammar/cheatsheets/badfile.txt")

        assert response.status_code == 400
        data = response.json()
        assert "Invalid file path: badfile.txt" in data["detail"]

    async def test_get_cheatsheet_content_file_not_found(self, client_with_mock_grammar_service):
        ...
        response = await client.get("/api/grammar/cheatsheets/nonexistent.md")

        assert response.status_code == 404
        data = response.json()
        assert "Cheatsheet 'nonexistent.md' not found" in data["detail"]

    async def test_get_cheatsheet_content_service_error(self, client_with_mock_grammar_service):
        """Test handling of unexpected service errors."""
        client, mock_service = client_with_mock_grammar_service
        mock_service.get_cheatsheet_content.side_effect = Exception("Unexpected error")

        response = await client.get("/api/grammar/cheatsheets/test.md")

        assert response.status_code == 500
        data = response.json()
        assert "Failed to retrieve cheatsheet content" in data["detail"]

    async def test_grammar_endpoints_not_found_without_router(self, client_with_mock_grammar_service):
